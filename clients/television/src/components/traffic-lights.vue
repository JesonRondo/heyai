<template>
  <div class="wrap" v-bind:class="{focus: isFocus}">
    <div class="traffic-lights">
      <button class="traffic-light traffic-light-close" @click="onCloseClick"></button>
      <button class="traffic-light" v-bind:class="{'traffic-light-minimize': !isFullscreen}"
        @click="onMinimizeClick"></button>
      <button class="traffic-light traffic-light-maximize" @click="onMaximizeClick"></button>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    win.on('blur', () => this.isFocus = false)
    win.on('focus', () => this.isFocus = true)
    win.on('enter-fullscreen', () => this.isFullscreen = true)
    win.on('restore', () => this.isFullscreen = false)
  },
